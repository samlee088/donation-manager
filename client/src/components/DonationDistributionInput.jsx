...
const DonationDistributionInput = ({
  ...
  const distributionSubmissionHandler = async (event) => {
    ...
    try {
      ...
      const distributionCategoryInventory = currentInventory.find(
        (inventory) => inventory.inventoryCategory === distributionCategory
      );
      ...
      if (distributionQuantity > distributionCategoryInventory.inventoryValue) {
        ...
        if (
          addDistributionResponse.status >= 200 &&
          addDistributionResponse.status < 300
        ) {
          transactionStatus(
            `Distribution of quantity ${distributionQuantity} successful`
          );
        } else {
  ...
  return (
    ...
    >
      ...
      <Button
        variant="outlined"
        sx={{ margin: 2 }}
        onClick={distributionSubmissionHandler}
      >
        Submit Distribution
      </Button>
    </Box>
